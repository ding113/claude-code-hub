...
describe("proxy bare-path / nested-v1beta rewrites (server-side transparent)", () => {
  ...
  it("passes through /v1beta paths untouched (Gemini native route)", async () => {
    const { default: proxyHandler } = await import("@/proxy");
    const response = proxyHandler(makeRequest("/v1beta/models", "POST"));
    expect(response.headers.get("x-middleware-rewrite")).toBeNull();
    expect(response.status).toBeLessThan(300);
  });
});
