...
describe("SystemSettings：数据库缺列时的保存兜底", () => {
  ...
  test("getSystemSettings 在仅缺 cache_effectiveness_enabled 新列时应降级读取", async () => {
    ...
    // 第二次先剥离更新的 sessionSnapshotStore，第三次再剥离 cacheEffectivenessEnabled。
    const selectMock = vi
      .fn()
      .mockReturnValueOnce(createRejectedThenableQuery({ code: "42703" }))
      .mockReturnValueOnce(createRejectedThenableQuery({ code: "42703" }))
    ...
    // 关键回归保护：降级链必须按 migration 新旧顺序逐层剥离。
    const secondSelection = selectMock.mock.calls[1]?.[0] as Record<string, unknown>;
    expect(secondSelection).not.toHaveProperty("sessionSnapshotStore");
    ...
    expect(thirdSelection).not.toHaveProperty("sessionSnapshotStore");
