 ...
 * explicit session binding > prefix affinity hint > weighted random,
 * and an affinity hint must still pass the full hard validation.
 */
...
describe("ensure() nomination priority", () => {
  test("explicit session binding wins: affinity lookup is never consulted", async () => {
  ...
  test("flag off disables affinity entirely", async () => {
