...
export function toProvider(dbProvider: any): Provider {
  return {
    ...
    modelRedirects:
      dbProvider?.modelRedirects && Object.keys(dbProvider.modelRedirects).length > 0
        ? dbProvider.modelRedirects
        : null,
    ...
    keyPool: dbProvider?.keyPool ?? null,
    keySelectionStrategy: dbProvider?.keySelectionStrategy ?? "random",
