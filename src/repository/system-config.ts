  ...
  try {
    const [settings] = await db
      .select({
        ...
        enableAnthropicWarmupIntercept: systemSettings.enableAnthropicWarmupIntercept,
...
export async function updateSystemSettings(
  ...
  try {
    ...
    // Warmup 拦截配置字段（如果提供）
    if (payload.enableAnthropicWarmupIntercept !== undefined) {
      updates.enableAnthropicWarmupIntercept = payload.enableAnthropicWarmupIntercept;
    }
