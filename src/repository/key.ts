...
export async function findKeyById(id: number): Promise<Key | null> {
  const [key] = await db
    .select({
      ...
      fiveHourResetMode: keys.fiveHourResetMode,
      fiveHourResetAnchor: keys.fiveHourResetAnchor,
...
export async function createKey(keyData: CreateKeyData): Promise<Key> {
  const dbData = {
    ...
    fiveHourResetMode: keyData.five_hour_reset_mode ?? "rolling",
    fiveHourResetAnchor: keyData.five_hour_reset_anchor ?? null,
...
export async function updateKey(id: number, keyData: UpdateKeyData): Promise<Key | null> {
  ...
  if (keyData.five_hour_reset_mode !== undefined)
    dbData.fiveHourResetMode = keyData.five_hour_reset_mode;
  if (keyData.five_hour_reset_anchor !== undefined)
    dbData.fiveHourResetAnchor = keyData.five_hour_reset_anchor;
...
export async function validateApiKeyAndGetUser(
  ...
  if (cachedKey) {
    ...
    const [userRow] = await db
      .select({
        ...
        fiveHourResetMode: users.fiveHourResetMode,
        fiveHourResetAnchor: users.fiveHourResetAnchor,
  ...
  const result = await db
    .select({
      ...
      keyFiveHourResetMode: keys.fiveHourResetMode,
      keyFiveHourResetAnchor: keys.fiveHourResetAnchor,
      ...
      userFiveHourResetMode: users.fiveHourResetMode,
      userFiveHourResetAnchor: users.fiveHourResetAnchor,
  ...
  const user: User = toUser({
    ...
    fiveHourResetMode: row.userFiveHourResetMode,
    fiveHourResetAnchor: row.userFiveHourResetAnchor,
  ...
  const key: Key = toKey({
    ...
    fiveHourResetMode: row.keyFiveHourResetMode,
    fiveHourResetAnchor: row.keyFiveHourResetAnchor,
