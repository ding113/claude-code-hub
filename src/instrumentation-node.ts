/**
 * Node.js instrumentation implementation.
 * Loaded by src/instrumentation.ts only when NEXT_RUNTIME is nodejs.
 */
...
export async function register() {
  ...
  if (process.env.NEXT_RUNTIME === "nodejs") {
    ...
    if (process.env.NODE_ENV === "production") {
      ...
      // 初始化价格表（如果数据库为空）
      // react-doctor-disable-next-line react-doctor/async-parallel -- the cloud scheduler should start only after initial price seeding completes
      ...
      const [
        ...
        { startProbeScheduler, isSmartProbingEnabled },
      ] = await Promise.all([
        import("@/lib/log-cleanup/cleanup-queue"),
        import("@/lib/notification/notification-queue"),
        import("@/lib/circuit-breaker-probe"),
      ]);
      await Promise.all([scheduleAutoCleanup(), scheduleNotifications()]);
