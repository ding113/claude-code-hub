...
// rolling cost windows (5h / daily) 已退役：
//   - 限额拦截走 LeaseService（从 usage_ledger 索引上 SUM 取数）
//   - 展示走 cost-display-cache 短 TTL 字符串缓存 + DB SUM 回填
// 历史 TRACK_COST_*_ROLLING_WINDOW / GET_COST_*_ROLLING_WINDOW Lua 脚本因写路径
// O(N) 全量扫描问题（详见 docs/limit/redis-cost-rolling-slowlog-fix.md）已完全删除。
