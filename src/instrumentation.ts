...
async function startCacheEffectivenessScheduler(): Promise<void> {
  ...
  try {
    // 开关支持系统设置运行时覆写：调度器常驻，每 tick 检查有效开关
    const { isCacheEffectivenessEnabled } = await import("@/lib/system-settings/proxy-runtime");
    ...
    instrumentationState.__CCH_CACHE_EFFECTIVENESS_INTERVAL_ID__ = setInterval(() => {
      if (!isCacheEffectivenessEnabled()) return;
