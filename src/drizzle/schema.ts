...
export const providers = pgTable('providers', {
  ...
  // 启用后，如果该提供商配置了从非 claude 模型名到 claude-* 模型的重定向，可以加入 openai 调度池
  joinOpenAIPool: boolean('join_openai_pool').default(false),
  ...
  // - keyPool: 令牌池（JSON 数组），null 或空数组表示仅使用 key 字段
  // - keySelectionStrategy: 令牌选择策略，'random'（默认）或 'round_robin'
  keyPool: jsonb('key_pool').$type<string[] | null>().default(null),
  keySelectionStrategy: varchar('key_selection_strategy', { length: 20 })
    .notNull()
    .default('random')
    .$type<'random' | 'round_robin'>(),
...
// Heartbeat Settings table - 心跳全局配置
...
// Heartbeat URL Configs table - 心跳 URL 配置（支持多个URL独立配置）
export const heartbeatUrlConfigs = pgTable('heartbeat_url_configs', {
  ...
  heartbeatUrlConfigsEnabledIdx: index('idx_heartbeat_url_configs_enabled').on(table.isEnabled),
  heartbeatUrlConfigsProviderIdx: index('idx_heartbeat_url_configs_provider').on(table.providerId),
}));
