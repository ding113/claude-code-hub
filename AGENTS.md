...
- `bun dev` — starts Next.js (port 13500) with Turbo for local work.
- `bun build` / `bun start` — compiles the standalone production bundle and serves it.
- `bun lint`, `bun typecheck`, `bun format:check` — run ESLint 9, TypeScript `--noEmit`, and Prettier verification; treat failures as blockers.
- `bun db:generate`, `bun db:migrate`, `bun db:push`, `bun db:studio` — Drizzle Kit for schema evolution.
...
- Run `bun format` before submitting wide-ranging edits.

## Testing Guidelines

- Today we rely on `bun lint` and `bun typecheck` plus manual checks through `/admin`.
...
- Rebase onto `main`, run `bun lint && bun typecheck`, and flag anything that blocks deploy parity.
...
- Prefer `bun dev` with mock providers when debugging rather than production credentials.
