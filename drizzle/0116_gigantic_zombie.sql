...
-- AUTO_MIGRATE prebuilds these indexes concurrently outside the Drizzle transaction.
-- The marker makes this transactional fallback a no-op after a successful preflight.
DO $$
DECLARE
  v_marker CONSTANT text := 'cch:migration:0116:session-replay-index:v1';
BEGIN
  IF obj_description(to_regclass('idx_message_request_session_identity_created_at'), 'pg_class') IS DISTINCT FROM v_marker THEN
    DROP INDEX IF EXISTS "idx_message_request_session_identity_created_at";
    CREATE INDEX IF NOT EXISTS "idx_message_request_session_identity_created_at" ON "message_request" USING btree (COALESCE("session_identity", "session_id"),"created_at" DESC NULLS LAST) WHERE "message_request"."deleted_at" IS NULL;
    COMMENT ON INDEX "idx_message_request_session_identity_created_at" IS 'cch:migration:0116:session-replay-index:v1';
  END IF;

  IF obj_description(to_regclass('idx_usage_ledger_session_identity_created_at'), 'pg_class') IS DISTINCT FROM v_marker THEN
    DROP INDEX IF EXISTS "idx_usage_ledger_session_identity_created_at";
    CREATE INDEX IF NOT EXISTS "idx_usage_ledger_session_identity_created_at" ON "usage_ledger" USING btree (COALESCE("session_identity", "session_id"),"created_at" DESC NULLS LAST) WHERE "usage_ledger"."blocked_by" IS NULL AND "usage_ledger"."is_replay" = false;
    COMMENT ON INDEX "idx_usage_ledger_session_identity_created_at" IS 'cch:migration:0116:session-replay-index:v1';
  END IF;

  IF obj_description(to_regclass('idx_usage_ledger_user_created_at'), 'pg_class') IS DISTINCT FROM v_marker THEN
    DROP INDEX IF EXISTS "idx_usage_ledger_user_created_at";
    CREATE INDEX IF NOT EXISTS "idx_usage_ledger_user_created_at" ON "usage_ledger" USING btree ("user_id","created_at") WHERE "usage_ledger"."blocked_by" IS NULL AND "usage_ledger"."is_replay" = false;
    COMMENT ON INDEX "idx_usage_ledger_user_created_at" IS 'cch:migration:0116:session-replay-index:v1';
  END IF;

  IF obj_description(to_regclass('idx_usage_ledger_key_created_at'), 'pg_class') IS DISTINCT FROM v_marker THEN
    DROP INDEX IF EXISTS "idx_usage_ledger_key_created_at";
    CREATE INDEX IF NOT EXISTS "idx_usage_ledger_key_created_at" ON "usage_ledger" USING btree ("key","created_at") WHERE "usage_ledger"."blocked_by" IS NULL AND "usage_ledger"."is_replay" = false;
    COMMENT ON INDEX "idx_usage_ledger_key_created_at" IS 'cch:migration:0116:session-replay-index:v1';
  END IF;

  IF obj_description(to_regclass('idx_usage_ledger_provider_created_at'), 'pg_class') IS DISTINCT FROM v_marker THEN
    DROP INDEX IF EXISTS "idx_usage_ledger_provider_created_at";
    CREATE INDEX IF NOT EXISTS "idx_usage_ledger_provider_created_at" ON "usage_ledger" USING btree ("final_provider_id","created_at") WHERE "usage_ledger"."blocked_by" IS NULL AND "usage_ledger"."is_replay" = false;
    COMMENT ON INDEX "idx_usage_ledger_provider_created_at" IS 'cch:migration:0116:session-replay-index:v1';
  END IF;

  IF obj_description(to_regclass('idx_usage_ledger_key_cost'), 'pg_class') IS DISTINCT FROM v_marker THEN
    ...
    COMMENT ON INDEX "idx_usage_ledger_key_cost" IS 'cch:migration:0116:session-replay-index:v1';
  END IF;

  IF obj_description(to_regclass('idx_usage_ledger_user_cost_cover'), 'pg_class') IS DISTINCT FROM v_marker THEN
    ...
    COMMENT ON INDEX "idx_usage_ledger_user_cost_cover" IS 'cch:migration:0116:session-replay-index:v1';
  END IF;

  IF obj_description(to_regclass('idx_usage_ledger_provider_cost_cover'), 'pg_class') IS DISTINCT FROM v_marker THEN
    ...
    COMMENT ON INDEX "idx_usage_ledger_provider_cost_cover" IS 'cch:migration:0116:session-replay-index:v1';
  END IF;

  IF obj_description(to_regclass('idx_usage_ledger_key_created_at_desc_cover'), 'pg_class') IS DISTINCT FROM v_marker THEN
    DROP INDEX IF EXISTS "idx_usage_ledger_key_created_at_desc_cover";
    CREATE INDEX IF NOT EXISTS "idx_usage_ledger_key_created_at_desc_cover" ON "usage_ledger" USING btree ("key","created_at" DESC NULLS LAST,"final_provider_id") WHERE "usage_ledger"."blocked_by" IS NULL AND "usage_ledger"."is_replay" = false;
    COMMENT ON INDEX "idx_usage_ledger_key_created_at_desc_cover" IS 'cch:migration:0116:session-replay-index:v1';
  END IF;
